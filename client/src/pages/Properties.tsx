...
const Properties = () => {
  ...
  const [priceRangeSlider, setPriceRangeSlider] = useState<[number, number]>([0, 200000000000]) // 0 to 2000 Cr in rupees
  ...
  const [uniqueLocations, setUniqueLocations] = useState<{ city: string; areas: string[] }[]>([])
  ...
  useEffect(() => {
    ...
    if (priceMinParam && priceMaxParam) {
      setPriceRangeSlider([Number.parseInt(priceMinParam), Number.parseInt(priceMaxParam)])
    ...
    if (areaMinParam && areaMaxParam) {
      setAreaRange([Number.parseInt(areaMinParam), Number.parseInt(areaMaxParam)])
    }
  }, [])
  ...
  const extractUniqueValues = (properties: Property[]) => {
    ...
    // Extract amenities
    const allAmenities = properties.flatMap((property) => property.amenities || []).filter(Boolean)
  ...
  const updateURL = () => {
    ...
    // Advanced filters
    if (bedroomFilter.length > 0) params.set("bedrooms", bedroomFilter.join(","))
    if (bathroomFilter.length > 0) params.set("bathrooms", bathroomFilter.join(","))
    if (areaRange[0] > 0) params.set("areaMin", areaRange[0].toString())
    if (areaRange[1] < maxAreaValue) params.set("areaMax", areaRange[1].toString())
    if (priceRangeSlider[0] > 0) params.set("priceMin", priceRangeSlider[0].toString())
    if (priceRangeSlider[1] < maxPriceValue) params.set("priceMax", priceRangeSlider[1].toString())
    if (furnishingStatus.length > 0) params.set("furnishing", furnishingStatus.join(","))
    if (amenities.length > 0) params.set("amenities", amenities.join(","))
    if (propertyAge.length > 0) params.set("age", propertyAge.join(","))
    if (facingDirection.length > 0) params.set("facing", facingDirection.join(","))
    if (availabilityStatus.length > 0) params.set("availability", availabilityStatus.join(","))
  ...
  const parsePriceValue = (property: Property) => {
    ...
    try {
      ...
      const isLakh = property.price.toLowerCase().includes("lac") || property.price.toLowerCase().includes("lakh")
  ...
  const isCommercialType = (type?: string) => {
    return ["commercial", "office", "retail", "warehouse"].includes(type?.toLowerCase() || "")
  }
  ...
  const filteredAndSortedProperties = properties
    .filter((property) => {
      ...
      const matchesType = propertyType === "all-types" || property.type?.toLowerCase() === propertyType.toLowerCase()
      ...
      const matchesLocation = () => {
        ...
        if (property.locationData) {
          return (
            property.locationData.city.toLowerCase().includes(location.toLowerCase()) ||
            property.locationData.area.toLowerCase().includes(location.toLowerCase())
          )
        }
      ...
      const matchesPriceRange = () => {
        const priceValue = property.priceValue || 0
        return priceValue >= priceRangeSlider[0] && priceValue <= priceRangeSlider[1]
      }
      ...
      const matchesBathrooms = () => {
        ...
        if (bathroomFilter.includes("3+") && property.bathrooms >= 3) return true
      ...
      const matchesArea = () => {
        ...
        return property.areaValue >= areaRange[0] && property.areaValue <= areaRange[1]
      }
      ...
      const matchesFurnishing = () => {
        ...
        const hasFurnished =
          description.includes("furnished") || property.features?.some((f) => f.name.toLowerCase().includes("furnish"))
        const hasUnfurnished =
          description.includes("unfurnished") ||
          property.features?.some((f) => f.name.toLowerCase().includes("unfurnish"))
        ...
        if (furnishingStatus.includes("unfurnished") && hasUnfurnished) return true
        if (furnishingStatus.includes("semi-furnished") && hasSemiFurnished) return true
      ...
      const matchesPropertyAge = () => {
        ...
        if (
          ...
          (description.includes("under construction") || description.includes("upcoming"))
        ) {
          return true
        }
        ...
        if (
          ...
          (description.includes("new") || description.includes("brand new") || description.includes("newly built"))
        ) {
          return true
        }
        ...
        if (
          ...
          (description.includes("less than 5 years") || description.includes("< 5 years"))
        ) {
          return true
        }
        ...
        if (
          ...
          (description.includes("5-10 years") || description.includes("between 5 and 10 years"))
        ) {
          return true
        }
      ...
      const matchesFacing = () => {
        ...
        return facingDirection.some(
          (direction) =>
            ...
            property.features?.some((f) => f.name.toLowerCase().includes(`${direction.toLowerCase()} facing`)),
        )
      }
      ...
      const matchesAvailability = () => {
        ...
        if (availabilityStatus.includes("under-construction") && status.includes("construction")) {
          return true
        }
  ...
  const toggleBedroomFilter = (value: string) => {
    setBedroomFilter((prev) => (prev.includes(value) ? prev.filter((item) => item !== value) : [...prev, value]))
  }

  // Toggle bathroom filter
  const toggleBathroomFilter = (value: string) => {
    setBathroomFilter((prev) => (prev.includes(value) ? prev.filter((item) => item !== value) : [...prev, value]))
  }

  // Toggle furnishing status filter
  const toggleFurnishingStatus = (value: string) => {
    setFurnishingStatus((prev) => (prev.includes(value) ? prev.filter((item) => item !== value) : [...prev, value]))
  }
  ...
  const togglePropertyAge = (value: string) => {
    setPropertyAge((prev) => (prev.includes(value) ? prev.filter((item) => item !== value) : [...prev, value]))
  }

  // Toggle facing direction filter
  const toggleFacingDirection = (value: string) => {
    setFacingDirection((prev) => (prev.includes(value) ? prev.filter((item) => item !== value) : [...prev, value]))
  }

  // Toggle availability status filter
  const toggleAvailabilityStatus = (value: string) => {
    setAvailabilityStatus((prev) => (prev.includes(value) ? prev.filter((item) => item !== value) : [...prev, value]))
  }
  ...
  const AdvancedFilterDesktop = () => (
    <Popover>
      <PopoverTrigger asChild>
        <Button
          variant="outline"
          className={`flex items-center gap-2 ${isAdvancedFilterActive ? "border-amber-500 bg-amber-50 text-amber-700" : ""}`}
        >
      ...
      <PopoverContent className="w-[350px] p-0" align="start">
        ...
        <div className="p-4 max-h-[70vh] overflow-y-auto">
          <Accordion type="multiple" className="w-full">
                  ...
                  {["1", "2", "3", "4+"].map((value) => (
                    <Label
                      ...
                      className={`px-4 py-2 border rounded-md cursor-pointer ${
                        bedroomFilter.includes(value) ? "bg-amber-100 border-amber-500" : ""
                      }`}
                ...
                <div className="flex flex-wrap gap-2">
                  {["1", "2", "3+"].map((value) => (
                    <Label
                      key={`bathroom-${value}`}
                      className={`px-4 py-2 border rounded-md cursor-pointer ${
                        bathroomFilter.includes(value) ? "bg-amber-100 border-amber-500" : ""
                      }`}
            ...
            <AccordionItem value="furnishing">
              <AccordionTrigger className="py-3">Furnishing Status</AccordionTrigger>
              <AccordionContent>
                <div className="grid gap-2">
                  {["furnished", "semi-furnished", "unfurnished"].map((value) => (
                    <Label key={`furnish-${value}`} className="flex items-center gap-2 cursor-pointer">
                      <Checkbox
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    ...
                    <Label key={`amenity-${value}`} className="flex items-center gap-2 cursor-pointer">
                      <Checkbox checked={amenities.includes(value)} onCheckedChange={() => toggleAmenity(value)} />
                      <span>{value}</span>
                    </Label>
                  ))}
                </div>
              ...
              <AccordionContent>
                <div className="grid gap-2">
                  {[
                    ...
                    <Label key={`age-${item.id}`} className="flex items-center gap-2 cursor-pointer">
                      <Checkbox
            ...
            <AccordionItem value="facing">
              <AccordionTrigger className="py-3">Facing Direction</AccordionTrigger>
              <AccordionContent>
                <div className="grid grid-cols-2 gap-2">
                  {["North", "South", "East", "West", "North East", "North West", "South East", "South West"].map(
                    (value) => (
                      <Label key={`facing-${value}`} className="flex items-center gap-2 cursor-pointer">
                        <Checkbox
              ...
              <AccordionContent>
                <div className="grid gap-2">
                  {[
                    ...
                    <Label key={`availability-${item.id}`} className="flex items-center gap-2 cursor-pointer">
                      <Checkbox
                        checked={availabilityStatus.includes(item.id)}
                        onCheckedChange={() => toggleAvailabilityStatus(item.id)}
                      />
                      <span>{item.label}</span>
                    </Label>
                  ))}
        ...
        <div className="p-4 border-t">
          <Button className="w-full gradient-gold text-white" onClick={handleSearch}>
            Apply Filters
          </Button>
        </div>
      </PopoverContent>
    ...
    <Drawer open={isFilterDrawerOpen} onOpenChange={setIsFilterDrawerOpen}>
      <DrawerContent className="max-h-[90vh]">
        <DrawerHeader>
          ...
          <DrawerDescription>Apply filters to find your perfect property</DrawerDescription>
        </DrawerHeader>
          ...
          <Tabs defaultValue="basic" className="w-full">
            ...
            <TabsContent value="basic" className="space-y-4">
              <div className="space-y-4">
                ...
                <div>
                  <Label htmlFor="mobile-property-type" className="text-sm font-medium">
                    Property Type
                  </Label>
            ...
            <TabsContent value="advanced" className="space-y-4">
              <Accordion type="multiple" className="w-full">
                ...
                <AccordionItem value="area">
                  <AccordionTrigger className="py-3">Area (sq ft)</AccordionTrigger>
                  <AccordionContent>
                    <div className="px-2 pt-2 pb-6">
                      <Slider value={areaRange} min={0} max={maxAreaValue} step={100} onValueChange={setAreaRange} />
                ...
                <AccordionItem value="price-range">
                  <AccordionTrigger className="py-3">Price Range (₹ Crores)</AccordionTrigger>
                  <AccordionContent>
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-2">
                        <div>
                          <Label className="text-xs text-gray-500">Min (Cr)</Label>
                          <Input
                            type="number"
                            value={priceInputMin}
                        ...
                        <div>
                          <Label className="text-xs text-gray-500">Max (Cr)</Label>
                          <Input
                            type="number"
                            value={priceInputMax}
                      ...
                      <div className="px-2 pt-2 pb-6">
                        ...
                        />
                        <div className="flex justify-between mt-2 text-sm text-gray-500">
                          <span>{formatPriceForDisplay(priceRangeSlider[0])}</span>
                          <span>{formatPriceForDisplay(priceRangeSlider[1])}</span>
                        </div>
                      </div>
                    </div>
                  </AccordionContent>
                ...
                <AccordionItem value="furnishing">
                  ...
                  <AccordionContent>
                    <div className="grid gap-2">
                      {["furnished", "semi-furnished", "unfurnished"].map((value) => (
                        <Label key={`furnish-mobile-${value}`} className="flex items-center gap-2 cursor-pointer">
                          <Checkbox
                            checked={furnishingStatus.includes(value)}
                            onCheckedChange={() => toggleFurnishingStatus(value)}
                          />
                  ...
                  <AccordionContent>
                    <div className="grid grid-cols-1 gap-2">
                      {[
                        ...
                        <Label key={`amenity-mobile-${value}`} className="flex items-center gap-2 cursor-pointer">
                          <Checkbox checked={amenities.includes(value)} onCheckedChange={() => toggleAmenity(value)} />
                          <span>{value}</span>
                        </Label>
                      ))}
                  ...
                  <AccordionContent>
                    <div className="grid gap-2">
                      {[
                        ...
                        <Label key={`age-mobile-${item.id}`} className="flex items-center gap-2 cursor-pointer">
                          <Checkbox
                            checked={propertyAge.includes(item.id)}
                            onCheckedChange={() => togglePropertyAge(item.id)}
                          />
                ...
                <AccordionItem value="facing">
                  ...
                  <AccordionContent>
                    <div className="grid grid-cols-2 gap-2">
                      {["North", "South", "East", "West", "North East", "North West", "South East", "South West"].map(
                        (value) => (
                          <Label key={`facing-mobile-${value}`} className="flex items-center gap-2 cursor-pointer">
                            <Checkbox
                              checked={facingDirection.includes(value)}
                              onCheckedChange={() => toggleFacingDirection(value)}
                            />
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      {/* Header */}
      <section className="py-12 gradient-gold text-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <h1 className="text-4xl md:text-5xl font-bold mb-4">Properties</h1>
            <p className="text-xl">Discover your perfect property in Bhopal</p>
          </div>
        </div>
      </section>
    ...
    <AdvancedFilterDesktop />
          ...
          <div className="grid grid-cols-5 gap-4 mt-4">
            ...
            <div className="flex gap-2 col-span-2">
              <Button className="h-12 flex-1 gradient-gold text-white" onClick={handleSearch}>
            ...
            <Select value={sortBy} onValueChange={setSortBy} className="col-span-2">
              <SelectTrigger className="h-12">
        ...
        <div className="bg-white rounded-lg shadow-lg p-4 mb-6 md:hidden">
          <div className="flex gap-2 mb-4">
            <Input
              ...
              className="flex-1"
            />
            <Button variant="outline" className="shrink-0" onClick={() => setIsFilterDrawerOpen(true)}>
          ...
          <div className="mb-6 hidden md:block">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {propertyType !== "all-types" && (
                <Badge variant="secondary" className="bg-amber-100 text-amber-800">
                  Type: {propertyType}
                  <button className="ml-1" onClick={() => setPropertyType("all-types")}>
                    <X className="w-3 h-3" />
                  </button>
                </Badge>
              )}
              {priceRange !== "all-prices" && (
                <Badge variant="secondary" className="bg-amber-100 text-amber-800">
                  ...
                  {priceRange
                    ...
                    .join("")}
                  <button className="ml-1" onClick={() => setPriceRange("all-prices")}>
                    <X className="w-3 h-3" />
                  </button>
                </Badge>
              )}
              {location !== "all-locations" && (
                <Badge variant="secondary" className="bg-amber-100 text-amber-800">
                  Location: {location}
                  <button className="ml-1" onClick={() => setLocation("all-locations")}>
                    <X className="w-3 h-3" />
                  </button>
                </Badge>
              )}
              ...
              {bathroomFilter.length > 0 && (
                <Badge variant="secondary" className="bg-amber-100 text-amber-800">
                  ...
                  <button className="ml-1" onClick={() => setBathroomFilter([])}>
                    <X className="w-3 h-3" />
                  </button>
                </Badge>
              )}
              {(areaRange[0] > 0 || areaRange[1] < maxAreaValue) && (
                <Badge variant="secondary" className="bg-amber-100 text-amber-800">
                  Area: {formatAreaRange(areaRange[0])} - {formatAreaRange(areaRange[1])} sq ft
                  <button className="ml-1" onClick={() => setAreaRange([0, maxAreaValue])}>
                    <X className="w-3 h-3" />
                  </button>
                </Badge>
              )}
              {(priceRangeSlider[0] > 0 || priceRangeSlider[1] < maxPriceValue) && (
                <Badge variant="secondary" className="bg-amber-100 text-amber-800">
              ...
              {furnishingStatus.length > 0 && (
                <Badge variant="secondary" className="bg-amber-100 text-amber-800">
                  ...
                  <button className="ml-1" onClick={() => setFurnishingStatus([])}>
                    <X className="w-3 h-3" />
                  </button>
                </Badge>
              )}
              ...
              {facingDirection.length > 0 && (
                <Badge variant="secondary" className="bg-amber-100 text-amber-800">
                  ...
                  <button className="ml-1" onClick={() => setFacingDirection([])}>
                    <X className="w-3 h-3" />
                  </button>
                </Badge>
              )}
              {availabilityStatus.length > 0 && (
                <Badge variant="secondary" className="bg-amber-100 text-amber-800">
                  ...
                  <button className="ml-1" onClick={() => setAvailabilityStatus([])}>
                    <X className="w-3 h-3" />
                  </button>
                </Badge>
              )}
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">{filteredAndSortedProperties.length} Properties Found</h2>
        </div>
            ...
            >
              <div className="relative">
                <div className="relative group w-full h-64 overflow-hidden">
                  {/* Default image */}
                  <img
                    src={property.images[0]?.url || "/placeholder.svg?height=300&width=400" || "/placeholder.svg"}
                    alt={property.title}
                ...
                <div className="absolute top-4 left-4">
                  <Badge className={`${property.status === "For Sale" ? "bg-green-500" : "bg-blue-500"} text-white`}>
                    Available
                  </Badge>
                </div>
                ...
                <div className="border-t pt-4">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        <p className="text-sm font-medium text-gray-900">{property.vendor?.name || "Vendor"}</p>
